    ...
    async getInfo() {
        try {
            return await retry(async () => {
                ...
                let encodedData = ethers.utils.defaultAbiCoder.encode(["uint"], [amountWei]).substring(2);
    ...
    async refuel() {
        try {
            return await retry(async () => {
                ...
                const tx = await this.contract.bridgeGas(swapInfo.dstChainId, this.address, swapInfo.adapterParams, overrides);
