...
class Main {
    ...
    async sendRandomTransaction(protocol, wallet, network, walletNumber) {
        try {
    ...
    async runThread(wallet, walletNumber) {
        ...
        for (let i = 1; i <= randomTimes; i++) {
            ...
            const randomPick = await getRandomNetworkAndProtocol(balances);
            if (randomPick !== null) {
